...
_HeuristicEstimationPrompt = lambda q, p: """I am tasked to estimate the probability that a random solution to " """ + q+""" " has the following property " """+p+""" "
    Instructions:
...
class AutoStratification(curator.LLM):
    ...
    def prompt(self, input: dict) -> str:
        ...
        return [{"role": "user", "content": _AutoStratificationPrompt.format(user_request=question)}, 
...
class HeuristicEstimation1(curator.LLM):
    response_format = PropertyEvaluation
    
    def prompt(self, input: dict) -> str:
        """Generate a prompt for property generation."""
        return [{"role": "user", "content": _HeuristicEstimationPrompt(input["question"], input["property"])},  
            {"role": "system", "content": "You are an expert superforecaster, familiar with the work of Tetlock and others. Your mission is to generate accurate predictions for forecasting questions. Aggregate the information provided by the user. Make sure to give detailed reasoning."}]
    ...
    def collate(self, out):
        ...
        grouped_df = df.groupby("question").agg({
            "property": list,
            "probability": list,
            "reasoning": lambda x: " | ".join(x)
        }).reset_index()
        return Dataset.from_pandas(grouped_df)



class HeuristicEstimation2(curator.LLM):
    response_format = OptimalProperties
    
    def prompt(self, input: dict) -> str:
        """Generate a prompt for property generation."""
        properties = [ (pr, p) for pr, p in zip(input["property"], input["probability"])]
        return [{"role": "user", "content": _HeuristicEstimationPromptOptimal.format(question=input["question"], properties=properties)},  
            {"role": "system", "content": "You are an expert superforecaster, familiar with the work of Tetlock and others. Your mission is to generate accurate predictions for forecasting questions. Aggregate the information provided by the user. Make sure to give detailed reasoning."}]
...
class StratifiedGenerator:
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.heuristic1 = HeuristicEstimation1(*self.args, **self.kwargs)
        self.heuristic2 = HeuristicEstimation2(*self.args, **self.kwargs)
