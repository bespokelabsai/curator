...
from bespokelabs.curator.status_tracker.online_status_tracker import OnlineStatusTracker, TokenLimitStrategy, _TokenInfo
...
class LiteLLMOnlineRequestProcessor(BaseOnlineRequestProcessor):
    ...
    def _set_manual_tpm(self, config):
        if self.token_limit_strategy == TokenLimitStrategy.combined:
            ...
            self.manual_max_tokens_per_minute = _TokenInfo(**{"input": config.max_input_tokens_per_minute, "output": config.max_output_tokens_per_minute})
        else:
    ...
    def estimate_total_tokens(self, messages: list) -> _TokenInfo:
        ...
        return _TokenInfo(**{"input": input_tokens, "output": output_tokens})
    ...
    def get_header_based_rate_limits(self) -> tuple[int, _TokenInfo | int]:
        ...
        if output_token_key in headers:
            ...
            tpm = _TokenInfo(**{"input": input_tpm, "output": output_tpm})
